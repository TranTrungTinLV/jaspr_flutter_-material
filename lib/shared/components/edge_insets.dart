...
// ignore: uri_does_not_exist_in_doc_import
/// @docImport 'dart:ui';
///
// ignore: uri_does_not_exist_in_doc_import
...
import 'package:jaspr/dom.dart' hide Radius, Padding, BorderRadius, Color, Transform, Border, Text, BoxShadow, Shadow, Offset, VoidCallback;
import 'package:jaspr/dom.dart' as css show Padding, Unit;
// ignore: undefined_hidden_name
import 'package:jaspr/jaspr.dart' hide Padding, Transform, Color, Border, BorderRadius, Text , Offset;

import 'mock_types.dart' as ui show ViewPadding, lerpDouble;
...
abstract class EdgeInsetsGeometry {
  ...
  const factory EdgeInsetsGeometry.only({double left, double right, double top, double bottom}) = EdgeInsets.only;
  ...
  const factory EdgeInsetsGeometry.symmetric({double vertical, double horizontal}) = EdgeInsets.symmetric;

  /// Creates [EdgeInsets] from offsets from the left, top, right, and bottom.
  const factory EdgeInsetsGeometry.fromLTRB(double left, double top, double right, double bottom) = EdgeInsets.fromLTRB;
  ...
  /// Helper method for Jaspr to easily map to Padding (CSS)
  css.Padding get toJasprPadding {
    final resolved = resolve(TextDirection.ltr); // Assume LTR for simplicity on web
    return css.Padding.fromLTRB(
      css.Unit.pixels(resolved.left),
      css.Unit.pixels(resolved.top),
      css.Unit.pixels(resolved.right),
      css.Unit.pixels(resolved.bottom),
    );
  }

  /// Helper method for Jaspr to easily map to Margin (CSS)
  Margin get toJasprMargin {
    final resolved = resolve(TextDirection.ltr);
    return Margin.fromLTRB(
      css.Unit.pixels(resolved.left),
      css.Unit.pixels(resolved.top),
      css.Unit.pixels(resolved.right),
      css.Unit.pixels(resolved.bottom),
